...
- **Gerçek zamanlı kod editörü** — Monaco Editor + Yjs CRDT (çakışma yok, aynı anda yazma)
...
### Editor Senkronizasyonu (Yjs CRDT — Gerçek Zamanlı)

CodeOrbit artık **Yjs CRDT** kullanarak gerçek zamanlı işbirlikçi düzenleme sağlıyor. Bu, iki kullanıcının aynı dosyada aynı anda yazmasına olanak tanır ve tüm değişiklikler otomatik olarak çakışma olmadan birleşir.

```javascript
// Her dosya için bir Y.Doc
const ydoc = new Y.Doc();
const ytext = ydoc.getText('monaco');

// MonacoBinding — Y.Text ↔ Monaco model otomatik senkronizasyonu
const binding = new MonacoBinding(ytext, model, new Set([editor]), null);

// Yerel değişiklik → STOMP üzerinden yayımlanır
ydoc.on('update', (update, origin) => {
    if (origin === provider) return; // kendi uyguladığımız güncellemeyi atla
    stompClient.publish({
        destination: '/app/code.send',
        body: JSON.stringify({
            roomId, filePath, clientId,
            type: 'yjs-update',
            update: toBase64(update) // base64 Uint8Array
        })
    });
});

// Gelen mesaj → Y.applyUpdate ile CRDT birleştirme
provider.onRemoteMessage = (msg) => {
    if (msg.type === 'yjs-update') {
        Y.applyUpdate(ydoc, fromBase64(msg.update), provider);
    }
};
```
...
| Tür | Yön | Açıklama |
|-----|-----|----------|
| `yjs-update` | Giden / Gelen | Artımlı CRDT güncellemesi (base64 `Uint8Array`) |
| `yjs-request` | Giden | Yeni istemci tam durum talep eder |
| `yjs-offer` | Gelen | Aktif peer (>2s) `encodeStateAsUpdate(doc)` ile yanıt verir |
| `yjs-full` | Giden | 5 saniyede bir periyodik tam durum anlık görüntüsü (DB'ye kaydedilir) |

**Özellikler:**
- **Echo önleme**: `clientId` filtresi — kendi mesajlarımızı görmezden gelir
- **STOMP bağlantı yarış durumu**: WebSocket bağlanmadan önce açılan dosyalar için sağlayıcı `onConnect`'te tembel oluşturulur
- **Çift sağlayıcı koruması**: Bir dosyaya tekrar dönüldüğünde mevcut sağlayıcı yeniden kullanılır
- **DB kalıcılığı**: `yjsState` (BYTEA) hem yeni istemcilerin hızlı yakınsaması hem de otomatik kurtarma için depolanır
...
| Zorluk | Çözüm |
|--------|-------|
| **Concurrent edit** | **Yjs CRDT** — aynı anda yazma tam destekli, otomatik çakışma çözümü. Artımlı güncellemeler STOMP üzerinden yayınlanır. |
...
| **İşbirliği** | Yjs CRDT + y-monaco + y-protocols |
...
- **Yjs CRDT**: Çakışma olmadan gerçek zamanlı işbirlikçi metin düzenleme
